...
class SurfaceMeshGenerator:
    ...
    def _buildContour(
        ...
        Note: at the rail edge (lf=1), the deck and bottom Z values differ
        (the rail has finite thickness), so no deduplication is needed there.
        ...
        for lf in lfs:
            y = float(lf) * hw
            z = rz + self._crossSection.getDeckHeight(t, float(lf))
            points.append([x, y, z])
        ...
        for lf in reversed(lfs):
            y = float(lf) * hw
            z = rz + self._crossSection.getBottomHeight(t, float(lf))
            points.append([x, y, z])
